...
router.post("/editProfile", isLoggedIn, upload.fields([
  ...
  try {
    ...
    const profile_imageFile = req.files['profile_image'];
    const cover_imageFile = req.file['cover_image']
    const status = 'open'
    ...
    if (existingRecruiter.length > 0 && existingRecruiter[0].user_id !== companyId) {
      ...
      await pool.query('UPDATE companies SET company_name = ?, email = ?, description = ?, profile_image = ?, cover_image = ? , company_video = ?, status = ? WHERE user_id = ?',[company_name, email, description, profile_imageFile, cover_imageFile, company_video, status, companyId]);
...
router.post("/addJob", isLoggedIn, async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO jobs (user_id, title, location, salary, description, date_posted, internship_duration, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [req.user.user_id, title, location, salary, description, datePosted, internship_duration, status]
    );
...
router.put('/updateJob/:jobId', isLoggedIn, async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE jobs SET title = ?, location = ?, salary = ?, status = ?, description = ?, internship_duration = ? WHERE job_id = ?',
      [title, location, salary, status, description, internship_duration, jobId]
    );
